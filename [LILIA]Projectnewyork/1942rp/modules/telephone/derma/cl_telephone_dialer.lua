...
local function openDialer(telent)
    ...
    function frame:AddNumber(n)
        if #self.dialNum == 4 then --If number is already 10 in lenght then stop.
            return
        end
    ...
    local function NewNumberForm()
        ...
        function newNum.finish:Think()
            if not newNum.name:IsEmpty() and not newNum.number:IsEmpty() and #newNum.number:GetText() == 4 then
                self:SetText("Finish")
            else
                self:SetText("Quit")
            end
