...
function MODULE:ResetBones(client)
    if not client:GetBoneCount() then return end
    for i = 0, client:GetBoneCount() do
        client:ManipulateBonePosition(i, Vector(0, 0, 0))
        client:ManipulateBoneAngles(i, Angle(0, 0, 0))
    end
end

function MODULE:Surrender(client)
    local manipulations = {
        ["ValveBiped.Bip01_R_UpperArm"] = Angle(30, 0, 90),
        ["ValveBiped.Bip01_L_UpperArm"] = Angle(-30, 0, -90),
        ["ValveBiped.Bip01_R_ForeArm"] = Angle(0, -130, 0),
    ...
    client.IsSurrendered = true
    for k, v in pairs(manipulations) do
        local boneid = client:LookupBone(k)
        if boneid then client:ManipulateBoneAngles(boneid, v) end
    end

    client:notify("You are surrendering")
    client:SetActiveWeapon(client:GetWeapon("lia_keys"))
    hook.Add("PlayerSwitchWeapon", "Surrender" .. client:SteamID64(), function(p) if p == client then return true end end)
end

function MODULE:UnSurrender(client)
    client.IsSurrendered = false
    self:ResetBones(client)
    client:notify("You are no longer surrendering")
    hook.Remove("PlayerSwitchWeapon", "Surrender" .. client:SteamID64())
end

function MODULE:ToggleAnimaton(client, crossing, class, deactivateOnMove)
    if crossing then
        client:SetNWBool("animationStatus", true)
        if class then client:SetNWString("animationClass", class) end
...
function MODULE:SetupMove(client, moveData, cmd)
    if client:GetNWBool("animationStatus") then
        ...
        if self:VelocityIsHigher(client, deactivateOnMove) then self:ToggleAnimaton(client, false) end
        if client:KeyDown(IN_DUCK) then self:ToggleAnimaton(client, false) end
        if client:KeyDown(IN_USE) then self:ToggleAnimaton(client, false) end
        if client:KeyDown(IN_JUMP) then self:ToggleAnimaton(client, false) end
    end
end