...
local PANEL = {}
function PANEL:Init()
    hook.Add("CalcView", "MainMenuView", function(ply, pos, angles, fov)
    ...
    lia.gui.char = self
    local backDropPos, backDropAng = strPosAngConv(MODULE.backDrop)
    ...
    function self:BlurIncreaser()
        ...
        local anim = Derma_Anim("BlurIncreaser", self, function(pnl, anim, dt)
    ...
    MODULE:Intro(self, function()
        ...
        function self.back.DoBack(this, group)
            function this.DoClick(this)
                ...
                if MODULE.view.pos ~= backDropPos or MODULE.view.ang ~= backDropAng then
                    local anim = Derma_Anim("CameraMoverBack", panel, function(pnl, anim, dt)
                ...
                if self.blur == 0 then self:BlurIncreaser() end
            end
        end
        ...
        local function changeScreen(callback)
            ...
            timer.Simple(MODULE.DevMode and 0 or 0.5, function()
                local group = group()
...
function PANEL:OnRemove()
    hook.Remove("CalcView", "MainMenuView")
    if self.cmdl then self.cmdl:Remove() end
