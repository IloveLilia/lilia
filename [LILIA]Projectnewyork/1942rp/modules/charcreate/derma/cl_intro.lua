...
function MODULE:Intro(panel, callback)
    ...
    sound.PlayFile(self.MainTheme, "noplay", function(station, errorID, fault)
        if station then
    ...
    local pos, ang = strPosAngConv(self.backDrop)
    self.view.pos = pos
    self.view.ang = ang + Angle(-90, 0, 0)
    ...
    function lp:Paint(w, h)
        ...
        draw.NoTexture()
        surface.SetMaterial(Material(SCHEMA.charcreate))
    ...
    dark:AlphaTo(0, 1, 2, function()
        dark:Remove()
        local time = MODULE.DevMode and 0 or 18
        text:SmoothText("New York, 1962", function()
            local anim = Derma_Anim("CameraMover", panel, function(pnl, anim, dt) self.view.ang = LerpAngle(dt * 0.008, self.view.ang, ang) end)
            anim:Start(time)
            panel.camMover = anim
            timer.Simple(time / 4, function()
                if not text then return end
                text:AlphaTo(0, 0.2, 0, function()
                    if not lp then return end
                    lp:AlphaTo(255, 1)
                    timer.Simple((time - time / 4) / 2, function()
                        if not lp or not text then return end
                        lp:AlphaTo(0, 1, 0, function()
                            callback()
                            text:Remove()
                            lp:Remove()
                            dark:Remove()
